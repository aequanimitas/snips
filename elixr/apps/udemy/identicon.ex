defmodule Elixr.Elixr.Identicon do
  ...
  def pick_color(%Elixr.Identicon.Image{hex: [r,g,b | _tail]} = image) do
    ...
    %Elixr.Identicon.Image{image | color: {r,g,b}}
  end
  ...
  def draw_image(%Elixr.Identicon.Image{color: color, pixel_map: pixel_map}) do
  ...
  def build_pixel_map(%Elixr.Identicon.Image{grid: grid} = image) do
    ...
    %Elixr.Identicon.Image{image | pixel_map: pixel_map}
  end

  def filter_odd_squares(%Elixr.Identicon.Image{grid: grid} = image) do
     grid = Enum.filter grid, fn({code, _index}) -> 
       rem(code, 2) == 0
     end
     %Elixr.Identicon.Image{image | grid: grid}
  end

  def build_grid(%Elixr.Identicon.Image{hex: hex} = image) do
    grid = hex
      |> Enum.chunk(3)
